...
build:
	go build $(LD_FLAGS) ./cmd/pg_buffer_viz

install:
	go install $(LD_FLAGS) ./cmd/pg_buffer_viz
...
DOCKER_TAGS=-t bonnefoa/pg_buffer_biz:latest \
	-t bonnefoa/pg_buffer_biz:$(shell git describe --tags)
